...
checkLastModified(Chan* c,ulong d){
	...
	// Fue error, por lo que esta corrupto
	return -1;
}
...
checkFiles(MyFiles *f){
	...
	for(i = 0; i < f->nfiles; i++){
		r = checkLastModified(f->files[i],f->mdate[i]);
		switch(r){
			case -1:
				f->validos[i] = 0;
				f->corrupto[i] = 1;
...
existDev(char* name){
	...
	for(i = 1; i < nfichtab+2; i++){
		if(strcmp(dsotab[i].name,name) == 0)
			return 1;
	}
...
// Actualizaria tamaños si se permiten que crezcan
static void
setSize(Chan* c){
	int i = c->qid.path - 2; // Hay un desfase de 2 ficheros, raiz y ctl
	if(i < 0 || i >= nfichtab)
...
dsoconcat(char** args, int nargs){
	...
	if(existDev(target)){
		qunlock(&fichLock);
		error("#Q: El dispositivo ya existe o no se puede crear alli");
	}
	...
	if(!waserror()){
		...
		for(i = 0; i < nargs-2; i++){
			...
			ficheros[nfichtab].corrupto[i] = 0;
...
dsopart(char** args, int nargs){
	...
	if(!waserror()){
		...
		ficheros[nfichtab].corrupto[0] = 0;
...
/* Se mantienen, comentadas, las lineas necesarias para
     actualizar el tamaño si se  permitiese
*/

static long
dsoconcatwrite(MyFiles *f, void *a, long n, vlong offset){
	int nfiles = f->nfiles;
	long escrito = 0, aux_r;
	//long newsize = offset;
	vlong newoff = offset;
	Dir* fs;
	int tamb = 1024;
	...
	if(i < nfiles-1){
		...
		for(j = i+1; (escrito < n) && (j < nfiles); j++){
			...
			aux_r = devtab[file->type]->write(file, a, towrite, 0);
			a = (char*)a + aux_r;
...
/*
	Los ficheros que componen el interleaving pueden crecer hasta
	que su tamaño sea un multiplo del tamaño de bloque
*/
static long
